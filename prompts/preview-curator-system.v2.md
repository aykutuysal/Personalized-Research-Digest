...
```json
{
  "body": "## The big picture\n\nIf you're still weighing PFA against cryoballoon for persistent AF, this week gives you one long-term follow-up and two mechanism papers that change the terms of the choice [1][2][3].\n\n## What to steal\n\n- **Read [2] before your next reconnection-mapping case.** It resets what \"durable isolation\" should mean at 12 months.\n- The rhythm-control timing data in [4] is the one to quote next time scheduling pushes back on early ablation.\n\nIf only one thing sticks this week: the reconnection map, not the acute-success number, is where the outcome is already written [2].",
