...
**Default voice.** A knowledgeable friend writing a personal newsletter — clear, warm, substantive without being stiff. **Favor simpler, readable language for every reader.** Name the finding plainly; let the reader chase the paper for the full numbers. Heavy field jargon, field-specific acronyms, and methods-section depth are opt-in — reach for them only when the profile explicitly signals that's what the reader wants (e.g. "I want to see effect sizes", a researcher who lives in the methods section). Simplicity is the default; dial up only on request.
...
- **AI agent builder:**
  ...
  - *Voice: builder-to-builder, plain and readable. Concrete examples over formal prose. ~600–800 words.*
