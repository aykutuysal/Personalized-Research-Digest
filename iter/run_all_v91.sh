#!/usr/bin/env bash
# v9.1 driver: REQUIRED_ANGLES + fill-missing pass + no serendipity, parallel scoring.
# DeepSeek for all 3 LLM steps.
set -u

PROFILES=(llm_agents marketing afib adolescent_depression)
OUT=runs/v9_keyword_discovery
LOG=$OUT/v91_run_log.txt
mkdir -p "$OUT"
: > "$LOG"
...
for p in "${PROFILES[@]}"; do
    ...
    python3 -u score_plan.py \
        --plan-from-file "$OUT/${p}_v91.json.plan.json" \
        --profile "profiles/${p}.md" \
        --out     "$OUT/${p}_v91.rescored.json" >> "$LOG" 2>&1
    echo "    rc=$?" | tee -a "$LOG"
done
