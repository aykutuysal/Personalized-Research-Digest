...
**Pre-commit checklist — run for EVERY query before you write it down.** A query is invalid unless it passes all 6 items:

1. **Specificity carrier identified.** Which side of the AND is the specificity carrier? Write it down in your head. If you can't point to one side, the query is invalid.
2. **Every disjunct in the specificity carrier passes the test.** Each OR term is a multi-word phrase, a proper noun/drug/benchmark, or a subject-dominant single word. If even ONE disjunct is a generic single word (`review`, `memory`, `benchmark`, `robustness`, `reliability`, `oversight`, `supervision`, `feedback`, `planning`, `reasoning`, `evaluation`, `treatment`, `management`, `intervention`, `experiment`, `conversion`, `emotional`, `informational`, `targeting`, `personalization`, `engagement`, `customization`), move it to the scoping side or remove it.
3. **Specificity carrier ≠ publication type or methodology.** The carrier must name a phenomenon, procedure, drug, construct, or artifact. NEVER use `"randomized controlled trial"`, `"meta-analysis"`, `"systematic review"`, `"cohort study"`, `"case-control"`, `"clinical trial"`, `"field experiment"`, `"A/B test"` as the specificity carrier — these are research methods and match every field that uses them. If you need to target reviews/trials, put them on the scoping side alongside a subject-specific AND clause.
4. **Specificity carrier ≠ a complication or generic clinical outcome from a broader field.** For clinical subjects, prefer disease-native procedures/drugs/anatomy over complications. For AFib: `"catheter ablation"`, `apixaban`, `"left atrial appendage closure"` ✓. Not: `"ischemic stroke"`, `"major bleeding"`, `"heart failure"` — these are entities with their own huge literature that happens to overlap with AFib.
5. **Scoping side is allowed to have generic words** (category disjunctions like `marketing OR consumer OR retail` or `adolescent OR teen OR youth`). The job of the scoping side is to pin the discipline, not narrow the topic — generic disjuncts are fine HERE.
6. **Polysemy check on carrier disjuncts.** Pick the 2 most generic-sounding disjuncts. For each, ask: "Does this word have a distinct meaning in a distant field?" If yes, replace with a phrase.

Quality bar: a plan of 15 queries where 12 are in the healthy band (30–500 hits) with mostly-on-topic top-10 results is worth more than 18 queries where 10 are broad. Prefer dropping an angle to emitting a polluted query.
