...
- **Recall safety (500–2000/week)** → **REQUIRED**: your plan MUST include at least 2–3 queries that deliberately target this band. These are "big net" queries that catch papers your narrow queries miss. Do not skip this quota even if your subject feels narrow.
- **Too broad (>2000/week)** → NARROW with another AND constraint.

**Mandatory recall quota — read carefully.** A plan of 15 queries where every query lands in "healthy" (30–500) or "low" (<30) will under-recall the literature. Narrow queries individually retrieve few papers; their union is still small. You MUST include 2–3 queries that intentionally aim for the **recall_safety band (500–2000 hits/week)** to ensure the plan's total unique corpus reaches ≥800 papers/week. These broad queries typically use:
- The most generic phrasing of the core subject (e.g., `"atrial fibrillation"` alone paired with a broad topic side like `therapy OR treatment OR management`)
- Method-family disjunctions paired with a phrase-anchor bundle (e.g., `("adolescent depression" OR "youth depression" OR ...) AND (therapy OR treatment OR intervention OR psychotherapy OR medication)`)
- Core-subject × common-outcome queries (e.g., `("LLM agent") AND (performance OR evaluation OR deployment)`)

Skipping this quota is the #1 way plans end up under-recalling. If your subject is narrow (e.g., adolescent depression, rare disease, specific clinical condition), you STILL need these broad queries — they're how the plan reaches the corpus target. Expect slightly lower precision on these specific queries (~50-60%); that's the deliberate tradeoff. The narrow queries in the rest of your plan will carry the high-precision signal.
