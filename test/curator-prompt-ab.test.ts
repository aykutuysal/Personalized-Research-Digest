// test/curator-prompt-ab.test.ts
//
// A/B the curator system prompt against a fixed model (default: openai/gpt-5.4).
// Runs both prompts/preview-curator-system.md (v1 baseline) and
// prompts/preview-curator-system.v2.md (revised) N=3 times each against the
// same user prompt from test-curator-user.md and saves outputs to
// experiments/2026-04-22-curator-prompt-ab/.
//
// Gated by RUN_CURATOR_AB=1.
...
type Variant = { label: 'v1' | 'v2'; system: string }
...
describe('curator prompt A/B', () => {
  it('runs baseline vs revised prompt through the model', async () => {
    if (!process.env.RUN_CURATOR_AB) {
      console.log('Skipping — set RUN_CURATOR_AB=1 to run')
      return
    }
    ...
    const lines: string[] = []
    lines.push(`# Curator prompt A/B — ${new Date().toISOString()}`)
    lines.push('')
    lines.push(`Model: \`${MODEL_ID}\`  |  N per variant: ${N}  |  Total cost: $${totalCost.toFixed(6)}`)
    lines.push('')
    lines.push('| label | run | status | ms | in tok | out tok | cost | body chars |')
    lines.push('|---|---:|---|---:|---:|---:|---:|---:|')
