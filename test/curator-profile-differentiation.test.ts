// test/curator-profile-differentiation.test.ts
//
// Tests whether the current production curator prompt actually responds to
// different reader profiles — same pool, two opposed profiles (builder vs
// mechanism-researcher), N=3 each. If picks and body content barely diverge,
// the baseline curator is not doing reader-specific work.
//
// Gated by RUN_CURATOR_DIFF=1.
...
type Variant = { label: 'builder' | 'researcher'; userPrompt: string }
...
describe('curator profile differentiation', () => {
  it('runs baseline prompt against two opposed profiles', async () => {
    if (!process.env.RUN_CURATOR_DIFF) {
      console.log('Skipping — set RUN_CURATOR_DIFF=1 to run')
      return
    }
    ...
    const outDir = resolve(
      process.cwd(),
      process.env.CURATOR_DIFF_OUT ?? 'experiments/2026-04-22-profile-differentiation',
    )
    ...
    const userA = readFileSync(
      resolve(process.cwd(), 'test-curator-user.md'),
      'utf8',
    )
    const userB = readFileSync(
      resolve(process.cwd(), 'test-curator-user-b.md'),
      'utf8',
    )

    const variants: Variant[] = [
      { label: 'builder', userPrompt: userA },
      { label: 'researcher', userPrompt: userB },
    ]
    ...
    const runOne = async (v: Variant, i: number): Promise<Run> => {
      ...
      try {
        ...
        console.log(
          `[ok]   ${tag.padEnd(14)} ms=${ms.toString().padStart(6)} tokens=${
            result.totalTokens ?? '—'
        ...
        console.log(`[fail] ${tag.padEnd(14)} ms=${ms} ${message.slice(0, 160)}`)
        return result
      }
    }
    ...
    const lines: string[] = []
    lines.push(`# Curator profile differentiation — ${new Date().toISOString()}`)
    lines.push('')
