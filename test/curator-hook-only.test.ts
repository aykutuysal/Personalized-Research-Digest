// test/curator-hook-only.test.ts
//
// Final ablation: v2c (schematic full-body example) vs v2d (hook-only example).
// Isolates whether the MemEvoBench selection drift in v2/v2c comes from the
// decision-framing in the example's lede, or just from the hook demonstration.
//
// Runs both variants against openai/gpt-5.4 N=3 each, saves to
// experiments/2026-04-22-curator-hook-only-example/.
//
// Gated by RUN_CURATOR_HOOKONLY=1.
...
type Variant = { label: 'v2c' | 'v2d'; system: string }
...
describe('curator hook-only example ablation', () => {
  it('runs v2c vs v2d against the model', async () => {
    if (!process.env.RUN_CURATOR_HOOKONLY) {
      console.log('Skipping — set RUN_CURATOR_HOOKONLY=1 to run')
      return
    }
    ...
    const v2d = readFileSync(
      resolve(process.cwd(), 'prompts/preview-curator-system.v2d.md'),
      'utf8',
    )
    ...
    const lines: string[] = []
    lines.push(`# Curator hook-only example ablation — ${new Date().toISOString()}`)
    lines.push('')
